...
def normalize_quarter(raw_value: str) -> str:
    ...
    if not re.fullmatch(r"20\d{2}Q[1-4]", quarter):
        raise ValueError(
            "Dönem biçimi geçersiz. Örnek kullanım: 2023Q4"
        )

    if quarter not in VALID_QUARTERS:
        raise ValueError(
            f"Bu çalışma için izin verilen dönemler: "
...
def save_outputs(
    ...
    print(
        "Duplicate accession-holding satırı:",
        duplicate_holding_ids,
    )
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"Dönem: {quarter}")
    print(f"Kaynak klasör: {paths['extract_dir']}")
